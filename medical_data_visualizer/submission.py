...
def draw_cat_plot():
  ...
  df_cat = pd.melt(df, id_vars=['id', 'cardio'], value_vars=selected_columns)

  # Group and reformat the data to split it by 'cardio'. Show the counts of each feature. You will have to rename one of the columns for the catplot to work correctly.
  grouped_df = df_cat.groupby(['cardio', 'variable', 'value'], as_index=False).size()
...
def draw_heat_map():
  ...
  sns.heatmap(corr, mask=mask, annot=True, fmt=".1f", linewidths=.5, ax=ax, square=True, center=0, vmin=-0.1, vmax=0.25, cbar_kws={'format': '%.1f'})
